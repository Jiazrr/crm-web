  ...
  <div class="table-box">
    ...
    >
      <template #tableHeader="scope">
        ...
        <el-button type="danger" :icon="Delete" :disabled="!scope.isSelected" v-hasPermi="['sys:customer:remove']" @click="deleteCustomer(scope.selectedListIds)"
          >批量删除</el-button
        >
      </template>
      <!-- 表格操作 -->
      <template #operation="scope">
        ...
        <el-button type="danger" link :icon="Delete" v-hasPermi="['sys:customer:remove']" @click="deleteCustomer(scope.row.id)">删除</el-button>
...
const { refreshFlag } = storeToRefs(customerStore)
...
//删除选中客户
const deleteCustomer = async (ids: any) => {
  const idList = Array.isArray(ids) ? ids : [ids]
  if (idList.length === 0) return
  await useHandleData(CustomerApi.remove, idList, '删除所选客户')
  proTable.value.clearSelection()
  proTable.value.getTableList()
}
